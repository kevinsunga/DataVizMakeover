...
title: "Assignment"
description: |
  I use interactive visual techniques using R to solve [mini-challenge 2 of the
  2021 Vast Challenge](https://vast-challenge.github.io/2021/MC2.html).
author:
  - name: Kevin Sunga
    url: {}
date: 07-24-2021
...
a methodology I adapted for my assignment.
...
packages = c("raster", "sf", "clock", "tmap",
             "tidyverse", "leaflet", "geosphere", "sqldf",
             "plotly", "lubridate", "viridis", "ggraph",
             "igraph", "DT")
...
to get hour of day and day of week. I use these fields later for visualizing
...
<center>
### Interactive Maps with POIs by Date (filtered for 2 dates)
</center>

```{r}
gps_dots_selected <- sf_poi %>%
  filter(day_timestamp == "2014-01-06" | day_timestamp == "2014-01-18") %>%
...
I clean the credit card and loyalty card data and prepare to visualize both in
separate interactive data tables. I do this primarily because the credit card and
loyalty card data do not fully match up.
...
DT::datatable(df_loyalty_cleaned,
  ...
  formatStyle(0,
              ...
              lineHeight = '25%')
```
I surface the credit card and loyalty card information from df_cc_loyalty which 
contains the joined and filtered data. With this interactive table, it's possible
to enter a credit card and see what loyalty card it's associated with and vice versa.
Note that there's uncertainty with this method: it's possible for one card to be
associated with multiple cards.
...
DT::datatable(df_cc_loyalty_cleaned,
              ...
              caption = htmltools::tags$caption(
                style = 'caption-side: bottom; text-align: center;',
                'Table: ', htmltools::em('Credit Card and Loyalty card Pairs'))) %>%
  formatStyle(0,
              target = 'row',
              lineHeight = '25%')
```
...
were in the same location and time.
...
<center>
### Weekday Meet Ups Between 6 and 8 Hours Long
</center>
